      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          // Sensor Fusion Data, a list of all other cars on the same side of the road.
					...
					// Determine interpolated waypoints nearby
            ...
            // correct for wrap in s for spline interpolation (must be continuous)
					...
					// Determine car parameters and create vehicle object
          // Vehicle class values s,s_d,s_dd,d,d_d,d_dd
					double pos_s, s_dot, s_ddot;
					double pos_d, d_dot, d_ddot;
          
					// Values based on future points in previous path
					...
					double trajectory_start_time = subpath_size * PATH_DT;

					// default values if there is not enough previous points
					if (subpath_size < 4) {
						pos_x = car_x;
						pos_y = car_y;
						...
						// determine dx, dy vector from set of interpoated waypoints. Pos_x,pos_y are used as reference point

            
						int next_interpolated_waypoint_index = NextWaypoint(pos_x, pos_y, angle, interpolated_waypoints_x, 
																													interpolated_waypoints_y);
						double dx = interpolated_waypoints_dx[next_interpolated_waypoint_index - 1];
						double dy = interpolated_waypoints_dy[next_interpolated_waypoint_index - 1];
            ...
            // projection of xy velocity vector (V) onto S (sx,sy) and D (dx,dy) vectors
						s_dot = vel_x1 * sx + vel_y1 * sy;
						d_dot = vel_x1 * dx + vel_y1 * dy;

						// use another point to calculate s_ddot, d_ddot from xy acceleration
						...
						// differentiating trajectory coefficients
						double eval_time, pos_s2, pos_d2, s_dot2, d_dot2, s_ddot2, d_ddot2;
						vector<double> s_dot_coeffs = my_car.differentiate_coeffs(my_car.s_trajectory_coeffs);			
						vector<double> d_dot_coeffs = my_car.differentiate_coeffs(my_car.d_trajectory_coeffs);			
					  ...
					  pos_s2 = my_car.evaluate_coeffs_at_time(my_car.s_trajectory_coeffs, eval_time);		
						pos_d2 = my_car.evaluate_coeffs_at_time(my_car.d_trajectory_coeffs, eval_time);		
					...
					// Prediction generation from fusion data
					// Each car has the following data format:
          // id - unique identifier for the car
          // x - global map coordinate x
          // y - global map coordinate y
          // vx - velocity x
          // vy - velocity y
          // s - Frenet coordinate 
          // d  - Frenet coordinate 
					...
					for (auto sf: sensor_fusion) {
						...
						int v_id = sf[0];
						vector<vector<double>> preds = other_car.generate_predictions(trajectory_start_time, duration);
						predictions[v_id] = preds;
					}



					// Check whether it is any other car to left or right. If any restriction flag is used to limit available states to prevent crash
        		...
        		vector<vector<double>> target_s_and_d = my_car.get_target_for_state(state, predictions, duration, car_just_ahead);
						...
						vector<vector<double>> possible_trajectory = my_car.generate_trajectory_for_target(target_s_and_d, duration);

						double current_cost = calculate_total_cost(possible_trajectory[0], possible_trajectory[1], predictions);
 

						if (current_cost < best_cost) {
								best_cost = current_cost;
								best_frenet_trajectory = possible_trajectory;
								best_trajectory_state = state;
					...
					vector<double> coarse_s_trajectory, coarse_x_trajectory, coarse_y_trajectory, interpolated_s_trajectory, 
												 interpolated_x_trajectory, interpolated_y_trajectory;
					...
					if (subpath_size >= 2) {
						...
						coarse_x_trajectory.push_back(previous_path_x[subpath_size-2]);
						coarse_y_trajectory.push_back(previous_path_y[subpath_size-2]);
						coarse_s_trajectory.push_back(pos_s);
						coarse_x_trajectory.push_back(previous_path_x[subpath_size-1]);
						coarse_y_trajectory.push_back(previous_path_y[subpath_size-1]);
					} else {
						...
						coarse_y_trajectory.push_back(prev_y);
						coarse_s_trajectory.push_back(pos_s);
						coarse_x_trajectory.push_back(pos_x);
						coarse_y_trajectory.push_back(pos_y);
					}
					...
					coarse_y_trajectory.push_back(target_y1);
					...
					coarse_y_trajectory.push_back(target_y2);


					
					// Determine next s values
					...
					interpolated_x_trajectory = interpolate_points(coarse_s_trajectory, coarse_x_trajectory, interpolated_s_trajectory);
					interpolated_y_trajectory = interpolate_points(coarse_s_trajectory, coarse_y_trajectory, interpolated_s_trajectory);
					...
					// add new xy points from path
					for (int i = 0; i < interpolated_x_trajectory.size(); i++) {
